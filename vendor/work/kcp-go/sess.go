...
const (
	...
	cryptHeaderSize = randKeySize + crcSize
...
func newUDPSession(conv uint32, l *Listener, conn net.PacketConn, ownConn bool, remote net.Addr, block BlockCrypt) *UDPSession {
	sess := new(UDPSession)
...
func randInCrypt(buf []byte) {
	if len(buf) <= 4 {
		panic(errors.New("invalid kcp buf size"))
	}
	randKey := rand.Uint32()
	binary.LittleEndian.PutUint32(buf[:4], randKey)
	xor.Bytes(buf[4:], buf[4:], buf[:4])
}

func randDeCrypt(buf []byte) {
	if len(buf) <= 4 {
		return
	}
	xor.Bytes(buf[4:], buf[4:], buf[:4])
...
func (s *UDPSession) output(buf []byte) {
	...
	if s.block != nil {
		...
		binary.LittleEndian.PutUint32(buf[randKeySize:], checksum)
		randInCrypt(buf)
...
func (s *UDPSession) update() {
	...
	default:
		...
		SystemTimedSched.Put(s.update,
			time.Now().Add(time.Duration(s.kcp.interval)*time.Millisecond))
	}
}
	...
	if dataValid {
		...
		if s == nil && convValid { // new session
			if len(l.chAccepts) < cap(l.chAccepts) { // do not let the new sessions overwhelm accept queue
				s := newUDPSession(conv, l, l.conn, false, addr, l.block)
				s.kcpInput(data)
				l.sessionLock.Lock()
...
func ListenWithOptions(laddr string, block BlockCrypt) (*Listener, error) {
	...
	conn.SetReadBuffer(64 * 1024 * 1024)
...
func NewConn3(convid uint32, raddr net.Addr, block BlockCrypt, conn net.PacketConn) (*UDPSession, error) {
	return newUDPSession(convid, nil, conn, false, raddr, block), nil
}

// NewConn2 establishes a session and talks KCP protocol over a packet connection.
func NewConn2(raddr net.Addr, block BlockCrypt, conn net.PacketConn) (*UDPSession, error) {
	var convid uint32
	convLock.Lock()
	convID++
	convid = convID
	...
	return NewConn3(convid, raddr, block, conn)
}
...
func NewConn(raddr string, block BlockCrypt, conn net.PacketConn) (*UDPSession, error) {
	...
	return NewConn2(udpaddr, block, conn)
}
